  ...
  return (
    <main className="container-page grid min-h-[78vh] place-items-center py-16">
      <section className="surface w-full max-w-md p-8 sm:p-10">
        ...
        <h1 className="mt-4 text-4xl font-semibold tracking-[-0.04em] text-zinc-950">{t("registerTitle")}</h1>
        <div className="mt-8 grid gap-4">
          <label className="label">{t("name")}<input className="field" placeholder={t("name")} /></label>
          ...
          <button className="btn-primary w-full" type="button">{t("createAccount")}</button>
        </div>
        <p className="mt-6 text-center text-sm text-zinc-500">
          {t("alreadyAccount")} <Link href="/login" className="font-semibold text-zinc-950">{t("loginButton")}</Link>
        </p>
      </section>
    </main>
  );
